...
**遗留小项**（不影响正确性，可随后续票处理）：
- `ADR-0003` 写"每个条目一个 JSON 文件"，而 `content/config/realms.json` 是每集合一文件（config 集合的惯例，`docs/agents/content.md` 已说明）
- `apps/web/src/App.tsx` 的 UI 文案（"当前境界"等）仍硬编码，未按 ADR-0004 归入内容层；当前是 MVP 冒烟壳，随 UI 主题化票处理
